...
const getChannelUrl = async videoUrl =>
{
  ...
  .then(text => text
          ...
          .split('"')[0])
  .catch(error => console.error(error, 'When tried to fetch: ', videoUrl));
};
...
const setupUpdatingLink = video =>
{
  ...
  .then(channelNameElement =>
  {
    ...
    const channelLink = video.getAttribute('channelLink');
    if(!channelLink || channelLink === 'undefined')
      getChannelUrl(getVideoUrl(video))
      .then(url => 
        {
          linkTag.setAttribute('href', url)
          video.setAttribute('channelLink', url);
        }
      );
    else
      linkTag.setAttribute('href', channelLink)
